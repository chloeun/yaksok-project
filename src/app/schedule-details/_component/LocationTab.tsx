...
const LocationTab = ({ finalLocation, userId, scheduleId }: LocationTabProps) => {
  ...
  const [centerLatLng, setCenterLatLng] = useState<naver.maps.LatLng | null>(null); // 지도의 중심 좌표 상태
  ...
  useEffect(() => {
    const initMap = () => {
      let centerCoords: naver.maps.LatLng;

      // finalLocation이 존재할 경우 그 좌표를 사용, 없으면 기본 좌표 사용
      if (finalLocation) {
        naver.maps.Service?.geocode({ query: finalLocation.roadAddress }, (status, response) => {
          if (status === naver.maps.Service.Status.OK) {
            ...
            centerCoords = new naver.maps.LatLng(lat, lng);
            ...
            setCenterLatLng(centerCoords); // centerLatLng 설정
          } else {
        ...
        // 기본 서울 중심 좌표
        centerCoords = new naver.maps.LatLng(37.5665, 126.978);
        ...
        const mapInstance = new naver.maps.Map('map', mapOptions);
        setMap(mapInstance);
        setCenterLatLng(centerCoords); // 기본 서울 좌표로 설정
      }
    };
  ...
  const searchPlaces = async () => {
    ...
    if (!centerLatLng) {
      setError('검색을 위한 중심 좌표가 설정되지 않았습니다.');
      return;
    }
    ...
    console.log('검색 중심 좌표:', lat, lng); // 좌표 확인 로그 추가

    try {
      // Modify the API request to include the lat/lng as a filter for nearby places
      const apiUrl = `/api/search-place?query=${encodeURIComponent(query)}&lat=${lat}&lng=${lng}&radius=1000`;
      console.log('API 요청 URL:', apiUrl); // API 요청 URL 로그

      const response = await fetch(apiUrl); // Add a radius parameter (1000 meters in this case)
      const data = await response.json();

      if (response.ok && data.items && data.items.length > 0) {
        console.log('검색 결과:', data.items); // 검색 결과를 콘솔에 로그

        const searchResults: SearchResultItem[] = [];

        // 하트된 장소 불러오기 (Supabase)
        ...
        const heartedAddresses = heartedLocations
          ? heartedLocations.map((item: { location: { address: string } }) => item.location.address)
          : [];
